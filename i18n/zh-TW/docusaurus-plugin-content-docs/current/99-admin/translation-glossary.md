---
draft: true
slug: translation-glossary
id: admin-translation-glossary
title: 翻譯詞彙表
sidebar_position: 3
description: 三個 wiki 語言版本共用的標準 en / zh-CN / zh-TW 術語，讓翻譯保持一致。
---

# 翻譯詞彙表

本詞彙表定義 wiki 各處使用的標準英文／簡體中文（zh-CN）／繁體中文（zh-TW）術語。**英文原文**是事實來源；zh-CN 與 zh-TW 頁面必須使用這些確切術語以保持一致。

## 術語表格

| English | zh-CN (簡中) | zh-TW (繁中) |
|---------|--------------|--------------|
| Access Point (AP) | 存取點 | 存取點 |
| Monitor mode | 監聽模式 | 監聽模式 |
| Packet injection | 封包注入 | 封包注入 |
| Adapter (wireless) | 無線網絡卡 | 無線網絡卡 |
| Driver | 驅動程式 | 驅動程式 |
| Firmware | 韌體 | 韌體 |
| Firmware update | 韌體更新 | 韌體更新 |
| Troubleshooting | 疑難排解 | 疑難排解 |
| Compatibility matrix | 相容性矩陣 | 相容性矩陣 |
| Getting started | 快速入門 | 快速入門 |
| Wi-Fi adapter | Wi-Fi 無線網絡卡 | Wi-Fi 無線網絡卡 |
| Antenna gain | 天線增益 | 天線增益 |

## 規則

- **品牌與型號名稱絕不翻譯。** 讓 `ALFA`、`AWUS036ACM`、`HAK5`、`Flipper Zero`、`SDRLAB`、`TRX-duo`、`ACR1252U` 等保持與英文完全一致。
- **技術術語在翻譯頁面中的首次出現**應保留英文術語並附括號說明，例如 `monitor mode（監聽模式）`，之後再沿用翻譯後的術語。
- 英文來源頁面具有權威性。如果這裡缺少某個術語，請新增它，並在同一次變更中更新全部三個語言版本。

## 相關

- [`Getting Started`](/getting-started/) — 面向使用者的總覽。
- `Product Registry` — 標準產品名稱。