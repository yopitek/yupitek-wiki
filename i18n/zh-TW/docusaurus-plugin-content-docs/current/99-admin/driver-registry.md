...
title: 驅動程式登記表
sidebar_position: 2
description: Wiki 中每個已記錄的無線晶片組，對應到它所支援的 ALFA 網絡卡、驅動程式型別與內建於核心的狀態。
---

# 驅動程式登記表

驅動程式登記表把每個**無線晶片組**對應到使用它的 ALFA 網絡卡、**驅動程式型別**，以及現代 Linux 核心上的**內建於核心狀態**。這與 [`linux-compatibility-matrix`](/alfa-network/linux-compatibility-matrix/) 及 [`/alfa-network/drivers/`](/alfa-network/drivers/) 下的各晶片驅動程式指南互相呼應。

## 晶片組表格

| 晶片組 | 網絡卡 | 驅動程式繫結 | 內建於核心？ | 驅動程式指南 |
|---------|----------|----------------|------------|--------------|
...
- **內建於核心（mainline）** — 已包含在 Linux 核心中；網絡卡在最新核心上通常開箱即用，不需要額外安裝。MediaTek（`mt76x2u` / `mt76x0u` / `mt7921u`）網絡卡屬於此類。
...
每當驅動程式指南、網絡卡產品頁面或相容性矩陣被編輯時，請保持本登記表同步，讓交叉引用永遠不會脫節。