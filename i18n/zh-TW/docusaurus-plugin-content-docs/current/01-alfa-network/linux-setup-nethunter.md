...
title: ALFA 無線網卡在 NetHunter（Android）上——OTG 設定指南
sidebar_position: 6
description: 把手機變成行動 Wi-Fi 稽核設備——NetHunter + OTG + ALFA 無線網卡，含支援的晶片、驅動程式處理與驗證。
tags: [alfa, nethunter, android, otg, 監聽模式]
keywords: [NetHunter ALFA, Android OTG Wi-Fi 無線網卡, AWUS036ACM NetHunter, Kali NetHunter 設定]
---

# ALFA 無線網卡在 NetHunter（Android）上——OTG 設定指南

> **學習目標（Learning goal）**：完成本指南後，你的 ALFA 無線網卡將連接到執行 **Kali NetHunter** 的 Android 手機、在 NetHunter chroot 內可見，並準備好使用監聽模式（monitor mode）工具——全部由一條 USB **OTG** 傳輸線供電。
> **適用對象**：中階（需要 root 知識）｜ **前置需求**：已 root 的 Android 手機、已安裝 NetHunter、OTG 傳輸線、一支使用**內建於核心晶片**的 ALFA 無線網卡（見下方）。
...
**經驗法則**：對 NetHunter 而言，優先選擇 **AWUS036ACM（MT7612U）**。它是社群預設的 NetHunter 無線網卡，這是有原因的。
...
- [ ] OTG 傳輸線（依手機而定為 USB-C 或 micro-USB）——高功率無線網卡最好有外接電源
- [ ] 使用內建於核心晶片的 ALFA 無線網卡：**AWUS036ACM / AWUS036ACHM / AWUS036AXM / AWUS036AXML**
...
**預期輸出**：類似 `4.19.157-perf+`（較舊手機）或 `5.15.xx-gki`（較新）。MT7921AUN 無線網卡需要 **5.18 或更新**；MT7612U 在 4.19 上運作良好。

> **你可能會想問**——*「我需要特定的 NetHunter 核心嗎？」* 是的——NetHunter 團隊會為特定支援裝置清單建置核心。先查[官方裝置清單](https://www.kali.org/docs/nethunter/)：不支援的手機代表沒有支援監聽模式的核心，無論你做什麼，無線網卡都永遠離不開 managed 模式。

## 步驟 2：透過 OTG 連線

把 OTG 傳輸線插進手機，再把無線網卡插到 OTG 傳輸線上。多數手機會出現通知（「USB device connected」）。然後確認核心看到無線網卡：
...
**RTL8812AU（AWUS036ACH）**值得一段誠實的說明：它*可以*在核心包含預建 `8812au` 模組的 NetHunter 裝置上運作（某些社群核心有），但**不要把你的課程專案規劃在它上面**。在 Android chroot 內進行 DKMS 編譯在多數手機上會失敗，因為手機核心標頭檔不存在。如果你唯一的無線網卡是 Realtek，先在筆電上測試——[Kali 指南](/alfa-network/linux-setup-kali/) 在那裡可用——並把手機當成額外紅利。
...
| chroot 內 `iw dev` 為空 | 介面尚未建立 / 錯誤的 netns | 重新插上無線網卡；先檢查 `lsusb`；重新開機手機再重試 |
| `airmon-ng` 顯示 `command not found` | NetHunter chroot 不完整 | 透過 NetHunter 應用程式重新安裝 chroot；`apt update && apt install aircrack-ng` |
| 無線網卡偵測到但卡在 `managed` | 手機核心缺少該晶片的監聽支援 | 查 [NetHunter 支援裝置](https://www.kali.org/docs/nethunter/) 清單；改用內建於核心晶片的無線網卡 |
| MT7921AUN 無線網卡完全偵測不到 | 手機核心比 5.18 舊 | 使用較新的 NetHunter 核心映像，或使用 GKI 5.18+ 核心的手機 |
| 高負載下 WLAN 失效 | 手機的 USB 供電限制 | 供電 OTG hub；停用 NetHunter 的手機電池最佳化 |

## 參考資料

- [Kali Linux 桌面指南](/alfa-network/linux-setup-kali/)——完整的監聽 + 注入工作流程
- [Ubuntu 指南](/alfa-network/linux-setup-ubuntu/)——用戶端模式設定
- [相容性矩陣](/alfa-network/linux-compatibility-matrix/)——晶片 vs 作業系統表
- [Kali NetHunter 文件](https://www.kali.org/docs/nethunter/)——官方安裝與裝置支援