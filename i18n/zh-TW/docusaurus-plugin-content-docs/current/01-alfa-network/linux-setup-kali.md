...
title: ALFA 無線網卡在 Kali Linux 上——監聽模式與封包注入
sidebar_position: 5
description: ALFA 無線網卡的完整 Kali Linux 設定——Realtek 晶片的 DKMS 建置、監聽模式、封包注入驗證，以及常見錯誤表。
tags: [alfa, kali, 監聽模式, 封包注入, dkms, aircrack-ng]
keywords: [ALFA Kali, airmon-ng, Kali 監聽模式, 封包注入, AWUS036ACM Kali, RTL8812AU Kali]
---

# ALFA 無線網卡在 Kali Linux 上——監聽模式與封包注入

> **學習目標（Learning goal）**：完成本指南後，你的 ALFA 無線網卡將進入**監聽模式（monitor mode）**並**證明封包注入（packet injection）可用**——這是每個 Wi-Fi 稽核工具（Aircrack-ng、Wifite、Wireshark、Bettercap）都依賴的兩項能力。
> **適用對象**：初學者–中階、Kali 使用者 ｜ **前置需求**：已安裝 Kali Linux（任何近期版本）、網路連線、你的 ALFA 無線網卡。

## 概念：為什麼 Kali 不一樣

Ubuntu 把 Wi-Fi 無線網卡當成*有禮貌的用戶端*：它們只擷取自己的流量。Kali 的整套工具鏈——Aircrack-ng、Reaver、Wifite——假設無線網卡能做兩件額外的事：

1. **監聽模式（monitor mode）**——擷取某個頻道上的*每一*個訊框，而不只是你的連線。
2. **封包注入（packet injection）**——傳送原始的自製訊框（deauth、probe requests、handshake replays）。

不是每顆晶片都能做到這兩件事。好消息是：除了 **AWUS036EACS** 之外的所有 ALFA 無線網卡都可以，而且內建於核心的 MediaTek 晶片完全不需要安裝驅動程式。
...
- [ ] `sudo` 權限與網路連線
- [ ] 你的 ALFA 無線網卡——先查[相容性矩陣](/alfa-network/linux-compatibility-matrix/)
...
**預期輸出**：你的無線網卡出現在 `lsusb` 中，且 `iw dev` 至少有一個 `Interface wlan0`（或 `wlan1`）。
...
重新插上無線網卡（或 `sudo modprobe <module>`）並確認介面出現：`iw dev`。
...
注入是每支 Wi-Fi 無線網卡的技能檢定。對空執行 Aircrack-ng 自我測試（它會廣播）：
...
**`30/30: 100%`**——這就是神奇的一行。它代表無線網卡注入了 30 個 probe requests 並聽到全部 30 個，證明在監聽模式下 TX 與 RX 都正常運作。如果你看到 `Failed` 或很低的百分比，代表無線網卡實際上沒有在注入——請見下方常見錯誤表。
...
| `Injection is working!` 但 `0/30` 回覆 | 無線網卡 TX 正常但 RX 過濾器壞了（某些 Realtek 驅動程式常見） | 嘗試鎖定頻道：`sudo iw wlan0mon set channel 6`；重測；升級 DKMS 驅動程式 |
| DKMS 建置失敗並顯示 "No rule to make target" | Kali 核心對 repo 快照來說太新 | `cd /opt/rtl8812au && sudo git pull && sudo make dkms_install` |
| 監聽模式幾分鐘後失效 | USB 省電 / 熱節流 | 使用供電 USB hub 或 USB 3.0 連接埠；`sudo iwconfig wlan0mon txpower 20` |
| `airmon-ng check kill` 弄斷了我的網路 | 預期行為——NetworkManager 被停止了 | 工作結束後 `sudo systemctl restart NetworkManager` |
| 無線網卡在 managed 模式正常，但 `iw` 中沒有列出 `monitor` | 驅動程式建置時未含監聽支援 | 用 aircrack-ng repos 重建（它們啟用 monitor + VIF） |

> **你可能會想問**——*「擷取這些東西合法嗎？」* 監聽模式與注入是技術能力，不是執照。在大多數司法管轄區，在你不擁有或未獲授權測試的網路上擷取或注入是非法的。每個課程實驗室都假設你在**自己的 AP 或實驗室網路**上測試。請留在那裡。

## 參考資料

- [本指南的 Ubuntu 版本](/alfa-network/linux-setup-ubuntu/)——用戶端模式設定
...
- [Aircrack-ng 文件](https://www.aircrack-ng.org/doku.php)——官方工具文件