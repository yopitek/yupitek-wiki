...
title: ALFA Network
sidebar_position: 1
description: ALFA Network Wi-Fi 無線網卡、天線、Linux 驅動程式、Kali/NetHunter 設定指南、硬體整合與相容性矩陣。
tags: [alfa, wifi, kali, 監聽模式, 無線網卡, 天線]
keywords: [ALFA Network, AWUS036ACM, AWUS036AXML, Wi-Fi 無線網卡, 監聽模式, Kali Linux]
---

# ALFA Network

> **一句話定位（One-liner）**：ALFA Network 生產滲透測試人員、無人機飛手與機器人實驗室會優先選擇的外接 Wi-Fi 無線網卡與天線——因為它們結合了**高增益無線電**、**外接天線接頭**，以及（學生們最愛的部分）**扎實的 Linux 支援**。

如果你曾看過某個 Kali Linux 教學影片，片中有人把一支黑色棒子插進 USB 連接埠、把介面切換到監聽模式（monitor mode）然後開始嗅探封包，那支棒子幾乎可以肯定是 ALFA。這個品牌十多年來一直是資安社群中的預設選擇——從傳奇的 **AWUS036ACH** 到全新的 **Wi-Fi 6E AWUS036AXML**。

這本 wiki 是你一步一步的最佳夥伴：我們銷售的每一款無線網卡與天線、每一顆晶片的驅動程式、每一個「為什麼我的無線網卡沒出現」的解答，以及 Jetson、Raspberry Pi 與 Unitree 機器人的硬體整合指南。
...
| 頁面 | 內容 |
|---|---|
| [Wi-Fi 無線網卡比較](/alfa-network/wifi-adapter-comparison/) | 每款 USB 無線網卡並排比較：晶片、速度等級、頻段、監聽模式支援——以及依使用情境的「哪一款適合你」 |
| [Linux 相容性矩陣](/alfa-network/linux-compatibility-matrix/) | 哪些無線網卡在 Kali Linux、Ubuntu 與 NetHunter/Android 上可開箱即用 |
...
| [NetHunter（Android）設定指南](/alfa-network/linux-setup-nethunter/) | 把手機 + OTG 變成行動稽核設備 |
...
| [產品](/alfa-network/products/awus036acm/) | 每款無線網卡與天線的完整規格表與個別產品指南 |

## 如何選擇無線網卡（5 分鐘速成）

選擇無線網卡其實就是選擇**晶片**，因為晶片決定了：
...
如果這是你的第一支 ALFA 無線網卡，建議路徑如下：

1. 閱讀[比較](/alfa-network/wifi-adapter-comparison/)並挑選你的無線網卡。
