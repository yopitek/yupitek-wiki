...
sidebar_position: 3
description: ALFA APA-M25——10 dBi 雙頻 2.4/5 GHz 指向性面板天線，RP-SMA 接頭。ALFA 無線網卡的經典固定連結面板。
tags: [alfa, 天線, 面板, 雙頻]
...
> **一句話定位（One-liner）**：**APA-M25** 是 ALFA 經典的 **2.4 與 5 GHz 10 dBi 雙頻指向性面板**，以 **RP-SMA** 終接。它是任何配外接天線的 ALFA 無線網卡的「指向 AP 就贏」升級——校園中繼專案就是建立在這種面板上。
...
| 極化 | 線性、垂直 |
| 設計 | 緊湊扁平面板，壁掛/桅杆安裝 |
| 使用情境 | 固定點對點連結、長距離用戶端連結、涵蓋聚焦 |

## 總覽

APA-M25 是大多數人想到「ALFA 指向性天線」時浮現的面板：一面扁平、耐天候的矩形，把任何配外接天線的 ALFA 無線網卡變成長距離連結工具。它在 **2.4 與 5 GHz 上都是 10 dBi**，這是可以用肉眼瞄準的面板的實用上限——再上去，天線會變大、變窄、變得不留情面。

先注意兩件事：

1. **RP-SMA（公）接頭**——它直接插進無線網卡的 **RP-SMA（母）** 天線連接埠，不需要轉接頭，因為針極性與無線網卡側的插座相符。（[APA-M25-6E](/alfa-network/products/apa-m25-6e/) 與 [APA-M04](/alfa-network/products/apa-m04/) 使用 PR-SMA，*也*能正確與 RP-SMA 連接埠配對——兩個家族都設計成能與 ALFA 無線網卡連接埠接合。）
...
1. 旋下無線網卡的原廠天線。
2. 把 APA-M25 鎖進無線網卡的 RP-SMA 連接埠——手指緊加上輕輕的四分之一圈。
3. 絕不要硬轉：無線網卡的中心針很脆弱，而且面板比偶極天線重，所以**盡可能用束線帶做應力釋放**支撐傳輸線。
...
- **2.4 GHz 上的頻道選擇**：面板聚焦能量但不聚焦干擾——在 2.4 GHz 上，先挑乾淨的頻道（`1/6/11`），再瞄準。
...
| 搭配 | 結果 |
|---|---|
| 任何配 RP-SMA 天線連接埠的 ALFA 無線網卡（ACM、ACH、ACS、AX、AXM、AXML...） | ✅ 直接鎖上 |
| 5 GHz 連結 | ✅ 完整 5 GHz 支援 |
| 6 GHz（Wi-Fi 6E）連結 | ❌ 請用 [APA-M25-6E](/alfa-network/products/apa-m25-6e/) |
| 整合式天線的無線網卡（AXER、EACS） | ❌ 沒有可連接的連接埠 |
...
| 只看到 2.4 GHz 網路 | 無線網卡/法規領域問題，不是天線 | `sudo iw reg set <CC>`；用原廠天線測試以隔離 |
...
- [無線網卡比較](/alfa-network/wifi-adapter-comparison/)——搭配正確的無線電
