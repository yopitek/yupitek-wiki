...
sidebar_position: 6
description: ALFA AWUS036ACH——傳奇的高功率 RTL8812AU AC1200 雙頻 USB Wi-Fi 無線網絡卡。Kali 監聽模式、封包注入、DKMS 設定指南。
tags: [alfa, 無線網絡卡, rtl8812au, ac1200, kali, 監聽模式]
keywords: [AWUS036ACH, RTL8812AU, 高功率無線網絡卡, Kali Linux 無線網絡卡]
---

# ALFA AWUS036ACH——高功率 AC1200

> **一句話定位（One-liner）**：**AWUS036ACH** 是 ALFA 最有名的無線網絡卡——一支**高功率 RTL8812AU AC1200 雙頻** USB 3.0 dongle，配兩支 5 dBi 外接天線。如果你曾在 Kali 教學影片中看過一支黑色 ALFA，大概就是它。監聽模式（monitor mode）與封包注入（packet injection）是它的拿手好戲；DKMS 驅動程式建置是唯一的一次性入場費。
...
AWUS036ACH 是為 ALFA 建立「駭客無線網絡卡」名聲的那支無線網絡卡。它把耐操的 **RTL8812AU** 無線電配上真正的**高功率（500 mW）前端**與兩支可更換的 5 dBi 天線，包在你一眼就認得的黃/黑 ALFA 外殼裡。對課程作業最重要的區別：與低價仿冒品不同，它的驅動程式背後有多年 aircrack-ng 社群打磨，所以一旦驅動程式就位，**監聽模式與封包注入就是能用**。
...
RP-SMA 連線埠接受任何 [ALFA 天線](/alfa-network/products/apa-m25/)——升級到面板或更高增益的偶極天線能提升 500 mW 無線電能觸及的範圍。
...
| 無線網絡卡只在 managed 模式 | 核心 stub 搶走了它 | 把 `rtl8812au` 加入黑名單（見[驅動程式頁面](/alfa-network/drivers/rtl8812au/)） |
