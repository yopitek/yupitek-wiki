...
title: ALFA 無線網絡卡在 Raspberry Pi 上（3 / 4 / 5）
sidebar_position: 2
description: 把 Raspberry Pi 變成 Wi-Fi 實驗室工作站——Pi OS 上的 ALFA 無線網絡卡設定、hostapd 存取點、監聽模式與 Pi 3/4/5 的電源管理技巧。
...
> **一句話定位（One-liner）**：Raspberry Pi + ALFA 無線網絡卡是經典的平價實驗室工作站：用內建於核心晶片的 ALFA **擷取流量**，或把 Pi 變成 **hostapd 存取點（access point）**，範圍是 Pi 內建無線電做夢都想不到的。

## 概念：為什麼 Pi 是完美的 ALFA 主機

Pi 的整合式 Wi-Fi 是單天線而且很弱——做 SSH 可以，嗅探或為整個房間提供 Wi-Fi 就沒用。外接 ALFA 無線網絡卡改變了這個局面：

- **監聽模式（monitor mode）實驗室工作站**：內建於核心的晶片（MT7612U 等）給你一個無頭擷取裝置做 Wireshark 課程作業——插上、`airmon-ng start`、完成。
- **存取點（hostapd）**：AWUS036ACM/ACH 搭配外接天線把 Pi 變成真正的 AP，範圍遠勝內建無線電。
- **雙無線電技巧**：Pi 內建 = 使用者端上行，ALFA = AP 下行。一臺 Pi，兩個網路。

各主機板的硬體注意事項：

| 主機板 | USB | 注意事項 |
|---|---|---|
| Pi 3 | USB 2.0 | 支援中最舊；AC433 等級或內建於核心的無線網絡卡沒問題 |
| Pi 4 | USB 2.0（共享匯流排） | 最受歡迎的選擇——高功率無線網絡卡請加供電 hub |
| Pi 5 | USB 3.0 + PCIe | 最快的 USB；AC1200/AX1800 吞吐量最佳 |

> ⚠️ **電源是 Pi 的頭號故障模式**。Pi 3/4 共享一條 USB 2.0 匯流排；一支 500 mW ALFA 加上鍵盤再加上其他東西，可能讓匯流排電壓不足。任何高於 AWUS036ACS 的無線網絡卡都請編列**供電 USB hub** 預算。
...
- [ ] ALFA 無線網絡卡——監聽模式強烈建議使用[內建於核心的晶片](/alfa-network/linux-compatibility-matrix/)
- [ ] 使用高功率無線網絡卡時準備供電 USB hub
...
**預期輸出**：你的無線網絡卡在 `lsusb` 中，介面類似 `wlan1`（Pi 的內建無線電通常是 `wlan0`）。如果 ALFA 是 Realtek，先安裝它的 DKMS 驅動程式——請見 [Ubuntu 指南](/alfa-network/linux-setup-ubuntu/)，在 Pi OS 上步驟完全相同。
...
**預期輸出**：`Active: active (running)`，使用者端現在可以加入 **alfa-lab**。
...
現在 `wlan0` 連你到網際網路，`wlan1` 服務實驗室。一臺 Pi，兩個網路。

## 疑難排解

| 症狀 | 原因 | 修復 |
|---|---|---|
| 無線網絡卡在筆電可用、在 Pi 上失效 | USB 供電限制 | 供電 USB hub；使用 Pi 官方 PSU；降低 `txpower` |
...
| 使用者端連上但沒有網際網路 | DHCP/NAT 未設定 | 啟用 NAT：`sudo iptables -t nat -A POSTROUTING -o wlan0 -j MASQUERADE` + `sysctl net.ipv4.ip_forward=1` |
...
- [hostapd 檔案](https://w1.fi/hostapd/)——官方 AP 守護程式檔案