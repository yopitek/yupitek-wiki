...
sidebar_position: 7
description: 詳解 OpenWrt (含 GL.iNet 路由器) 搭配 ALFA AWUS036AXML (MT7921U) 與 AWUS036ACM (MT7612U) 之套件安裝、無線配置與上游已知 Bug 應對策略。
...
> **技術摘要**：在 OpenWrt 嵌入式路由器（如 GL.iNet、x86 軟路由、Raspberry Pi 路由）上擴充 USB 無線網卡，能作為第二組獨立 WAN（WISP 無線中繼）、熱點 AP 或無線安全監聽節點。**ALFA AWUS036AXML**（MT7921U）自 OpenWrt 22.03.3（Kernel 5.10 backport）與 23.05（Kernel 5.15）起可透過套件庫直接安裝；**AWUS036ACM**（MT7612U）則全線內建原生支援。本文提供完整配置流程，並深度揭露 **上游 GitHub Issue #839（Active Monitor 當機）** 之限制與應對方法。
...
flowchart TD
    ...
    subgraph Adapters["ALFA 網卡晶片支援度"]
        ...
        RTL["AWUS036ACH (RTL8812AU)<br/>❌ 官方無內建套件 (需手動編譯)"]
    end
...
| ALFA 型號 | 核心晶片 | OpenWrt 22.03.3+ | OpenWrt 23.05+ | 必要安裝核心套件 |
|---|---|---|---|---|
...
| **AWUS036ACH** | Realtek RTL8812AU | ❌ 官方庫無提供 | ❌ 官方庫無提供 | 需自行用 SDK 編譯 out-of-tree 模組 |
...
OpenWrt 現行穩定版 (23.05) 之 LuCI 網頁管理介面對 6GHz (HE/EHT 頻寬) 尚未完全支援，AWUS036AXML 在 OpenWrt 下主要作為 2.4GHz 與 5GHz 高效能無線射頻運作。
