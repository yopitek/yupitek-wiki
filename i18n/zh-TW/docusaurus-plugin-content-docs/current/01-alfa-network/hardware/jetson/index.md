...
title: ALFA 無線網卡在 NVIDIA Jetson 上（Orin Nano / NX）
sidebar_position: 1
description: 在 NVIDIA Jetson Orin Nano 與 Orin NX 上使用 ALFA Wi-Fi 無線網卡——機器人與電腦視覺的 6 GHz Wi-Fi 6E 串流、監聽模式與核心需求。
tags: [alfa, jetson, nvidia, 機器人, wifi-6e, 監聽模式]
keywords: [Jetson Orin ALFA, Jetson Wi-Fi 6E, AWUS036AXML Jetson, 機器人無線]
---

# ALFA 無線網卡在 NVIDIA Jetson 上（Orin Nano / NX）

> **一句話定位（One-liner）**：你的 Jetson 是視覺電腦，不是路由器——所以它內建的 Wi-Fi 通常很弱而且是單頻。ALFA 無線網卡解決這個問題：**插上 AWUS036AXML 做 6 GHz Wi-Fi 6E 串流**，或任何內建於核心的 ALFA 做可靠的機器人遙測，甚至監聽模式（monitor mode）。

## 概念：為什麼 Jetson 需要外接無線網卡
...
- **穩定的高吞吐量連結**：AWUS036AXML（MT7921AUN）是 ALFA 產品線中唯一支援 6 GHz 的無線網卡，而且它的驅動程式自 5.18 起內建於核心。
...
**規則**：MT7921AUN 無線網卡需要 **JetPack 6**；經典 AWUS036ACM 兩者都可用。
...
- [ ] 網路（首次設定建議使用乙太網路）
- [ ] ALFA 無線網卡——要 6 GHz 請用 [AWUS036AXML](/alfa-network/products/awus036axml/)
...
> ⚠️ 記得 Jetson 特有的電源問題：Orin Nano 的 USB 連接埠可能供電受限。高功率無線網卡在高負載下，**供電的 USB hub** 是你的好朋友。也要留意 Jetson 的 `nvpmodel` 電源模式——降頻模式會降低 USB 穩定性。
...
| 攝影機負載下無線網卡斷線 | USB 供電限制 | 供電 hub；提高 `nvpmodel` 模式 |
...
- [NVIDIA Jetson 文件](https://docs.nvidia.com/jetson/)