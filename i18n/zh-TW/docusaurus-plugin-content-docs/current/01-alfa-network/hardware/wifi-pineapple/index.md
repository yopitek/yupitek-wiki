...
title: Hak5 WiFi Pineapple × ALFA Network 硬體整合指南
...
flowchart TD
    ...
    subgraph ALFA["ALFA AWUS036ACM 5GHz 擴充"]
        ...
        Antennas["2× RP-SMA 雙頻可拆卸天線<br/>(預設 ARS-NT5B7 5dBi/7dBi)"]
        Chipset --> KernelDriver
        KernelDriver --> Antennas
    end

    USBHost -->|USB 匯流排連接| Chipset
    KernelDriver --> GeneratedIface["產生系統介面: wlan3<br/>• 5GHz Recon 頻譜偵察<br/>• 5GHz PineAP 偽造熱點廣播<br/>• 5.8GHz Deauth / 封包注入"]
```
...
2. **重要供電鐵律**：WiFi Pineapple Mark VII 本體功耗約 3W~5W，加上 AWUS036ACM 在 5GHz 滿載發射時瞬間電流達 900mA (4.5W)，整機總功耗可達 9.5W。
   - ❌ **嚴禁**：使用筆記型電腦弱電 USB 埠（僅 500mA）或低品質 5V/1A 豆腐頭供電。
   - ✅ **推薦**：使用支援 5V/3A (15W) 或 QC/PD 協定的專用變壓器 / 行動電源。

### Step 2: SSH 終端檢測與介面驗證
透過 SSH 連線進入 WiFi Pineapple 控制台（預設 IP `172.16.42.1`）：
...
2. 進入 **Recon** 模組：
   - 介面選擇下拉選單將自動出現 `wlan3 (5GHz)`。
   - 勾選 **2.4 GHz + 5 GHz** 雙頻掃描，設定週期為 `Continuous` 或 `1 min`。
   - 點擊 **Start Scan**，系統將同時掃描 5GHz 頻道（36~165）之 AP 與連線 Client。
3. 進入 **PineAP** 模組：
...
### Q2: 介面卡在 `wlan2` 衝突，Web UI 看不到 5GHz？
