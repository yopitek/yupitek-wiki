...
> **一句話定位（One-liner）**：**MediaTek MT7612U** 是 **AWUS036ACM** 內部的晶片——它的 `mt76x2u` 驅動程式**自版本 4.19 起內建於 Linux 核心**。光是這個事實就讓 ACM 成為 Linux 上摩擦最小的 ALFA 無線網卡：不需要安裝、不需要 DKMS、不需要維護。
...
- **沒有 DKMS**：核心更新時沒有需要重建的東西。無線網卡不會像 Realtek DKMS 模組那樣「升級後壞掉」。
...
`mt76x2u` 那一行帶有非零的 refcount 代表驅動程式認領了你的無線網卡。（如果 `lsmod` 什麼都沒顯示但 `lsusb` 看得到裝置，驅動程式是隨需載入的核心模組——執行 `sudo modprobe mt76x2u`。）
...
> **你可能會想問**——*「為什麼我的 MAC 是 00:c0:ca:...？」* 因為 `00:c0:ca` 是 **ALFA MAC OUI**——每支 ALFA 無線網卡都以這三個位元組開頭。在塞滿 dongle 的實驗室裡識別無線網卡時很方便。
...
| 症狀 | 原因 | 修復 |
|---|---|---|
| `lsusb` 什麼都沒顯示 | 電源 / 傳輸線問題 | 試另一個連接埠、供電 hub、另一條傳輸線——請見[疑難排解索引](/alfa-network/troubleshooting/) |
...
| 暫停/恢復後 WLAN 消失 | 某些筆電上已知的 USB 怪癖 | 恢復後拔下/重新插上無線網卡 |
