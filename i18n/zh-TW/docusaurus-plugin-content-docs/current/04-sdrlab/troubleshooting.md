...
如果 `lsusb` 什麼都沒顯示，問題出在 USB 連接本身（纜線、連接埠、集線器）。如果*有*顯示裝置，請繼續。
...
### 症狀
你調諧到某個頻率，卻聽到*其他*頻率的電台混進來，尤其是在 HF。

### 根本原因
來自強力廣播電台的過載，或調諧到約 24 MHz 以下——舊式接收棒會在此折疊訊號（混疊）。RTL-SDR Blog V4 處理方式與舊接收棒不同——它內建 28.8 MHz 的 HF 升頻器，以及用於強 FM/DAB 干擾的三工器 + 陷波濾波器。
...
4. 完整的官方程序請見 [TRX-duo → 首次開機與網路](/sdrlab/hardware/trx-duo/#first-boot-and-network)。
...
### 診斷
- 檢查 RJ45 插孔上的乙太網路連線 LED——如果沒亮，問題在纜線／連接埠。
...
### 症狀
瀑布圖有反應，但你什麼都聽不到，即使強力 HF 廣播電台也一樣。

### 診斷
檢查輸入：天線有接在**RX1/RX2 SMA 連接埠**嗎？頻段正確嗎（10 kHz – 60 MHz）？

### 根本原因
沒有天線／接錯連接埠、輸入衰減器啟動，或接收應用程式的增益設為最低。
...
1. 充電直到充電指示燈顯示進度。
2. 按住電源按鈕約 3 秒。
3. 還是無法啟動？連接 USB-C，檢查電腦是否看到 HackRF 裝置——如果有，依照 [H4M → Mayhem 韌體](/sdrlab/hardware/h4m/#mayhem-firmware)重新燒錄 Mayhem。
...
### 症狀
插入纜線後，RJ45 連接埠的 LED 保持熄滅。

### 診斷
- 換一條纜線、換一個交換器連接埠試試（模組是 10/100——有些僅支援 1G 的「智慧」連接埠很挑剔）。
- 依 [Ethernet 模組頁面](/sdrlab/expansion/ethernet-test-module/#wiring-to-the-flipper)確認與 Flipper 的接線。

### 根本原因
纜線／連接埠不良，或 SPI 接線（CS/RESET）錯誤導致 W5500 從未初始化。
