...
> **一句話總結**：乙太網路測試模組把**10/100 乙太網路連線埠裝上你的 Flipper Zero**——一顆內建 TCP/IP 協定堆疊的 WIZnet W5500——把 Flipper 變成口袋型 LAN 測試器，用於纜線檢查、DHCP 驗證與 ping 測試。
...
- **網頁目錄**（瀏覽器 + WebUSB）：在 Chromium 瀏覽器中開啟 Flipper 應用程式目錄，連線 Flipper，按安裝。搜尋「W5500」或「Ethernet」。
