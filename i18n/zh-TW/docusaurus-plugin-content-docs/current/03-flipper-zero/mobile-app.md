...
description: 透過藍芽將 Flipper Zero 與官方 Flipper 手機應用程式配對 — 在 iOS 與 Android 上安裝、配對、同步資料、分享鑰匙、遠端控制與空中韌體更新。
sidebar_position: 3
tags: [flipper-zero, 手機應用程式, 藍芽, ble, ios, android, 配對]
keywords: [Flipper 手機應用程式, 藍芽配對, iOS, Android, 同步 Flipper, 遠端控制 Flipper]
authors: yupitek
date: 2026-08-21
...
> **學習目標**：完成後你將已安裝官方應用程式、透過藍芽配對 Flipper Zero、同步資料，並使用應用程式進行遠端控制與韌體更新。
>
> **適用物件**：擁有 iPhone 或 Android 手機的初學者。Flipper Zero 必須已開機運作（請參閱[快速入門](/flipper-zero/quickstart/)）。

Flipper Zero 的 STM32WB55 無線電處理器內建藍芽低功耗（BLE 5.4）。官方 **Flipper 手機應用程式**（由 Flipper Devices 開發）把你的手機變成這隻小海豚的大螢幕遙控器：瀏覽已儲存的鑰匙、與朋友分享、透過空中更新韌體，甚至從房間另一頭控制裝置。
...
請確認手機的藍芽已開啟，且手機距離 Flipper Zero 一兩公尺以內。

## 步驟 2：在 Flipper Zero 上啟用藍芽
...
> **你可能會被詢問「配對模式」** — 保留預設值。如果你之前配對過但失敗了，請在應用程式與手機的藍芽設定中取消配對，然後重試。

## 步驟 3：在應用程式中配對

1. 開啟 Flipper 手機應用程式。
2. 點選 **Connect**（應用程式會自動掃描附近的 Flipper Zero 裝置）。
3. 當 Flipper Zero 出現在清單中時，點選它。
...
> **為什麼需要 PIN 碼？** BLE 配對會保護連線——這與藍芽耳機配對的原理相同。如果 PIN 碼不符，Flipper Zero 與手機會拒絕連線。
...
應用程式的 **Remote Control** 分頁會在你的手機上映象 Flipper Zero 的 UI：

- 點選螢幕上的按鈕，取代 5 鍵方向鍵
- 從手機觸發「Bad USB」指令碼（鍵盤模擬）
...
| 錯誤 | 原因 | 修正 |
|---|---|---|
| 應用程式中找不到裝置 | Flipper 或手機的藍芽關閉 | 在 Flipper 上開啟 BLE（主選單 → Bluetooth），重新整理掃描 |
| 配對失敗／PIN 碼錯誤 | 先前的配對已過期 | 在應用程式與手機藍芽設定中取消配對，重新啟動兩者，重試 |
| 同步卡住 | 手機距離太遠 | 移動到 1–2 公尺內，重新啟動應用程式 |
| OTA 更新中途失敗 | BLE 連線中斷 | 讓手機保持靠近，重試；若反覆發生，改用 USB 連線的 qFlipper |
