...
把它想成數位系統的瑞士刀：一台口袋大小的裝置，可以**讀取**訊號、**儲存**訊號、**重播**訊號，並**模擬**門禁卡或遙控器——還能透過 GPIO 接腳充當微控制器的除錯器。
...
- **[手機應用程式](/flipper-zero/mobile-app/)** — 透過藍牙配對、同步資料，並從手機更新韌體。
...
| **Video Game Module** | RP2040 擴充模組：玩懷舊遊戲、將螢幕鏡像到電視、動作感測 | [產品頁面](/flipper-zero/products/video-game-module/) |
...
- **藍牙 5.4** — 連接到手機應用程式，進行遠端控制、資料分享與空中韌體更新。

## 使用 Flipper Zero 合法嗎？

在大多數地區，擁有並使用 Flipper Zero 進行研究、教育與測試自己的設備是合法的。然而，各國法律不同：干擾你不擁有的系統（開啟別人的車庫門、複製你未獲授權測試的門禁卡）可能違法。**只在你自己的裝置上測試，或取得明確授權。** 這同樣適用於 WiFi Devboard 與 Video Game Module。

> 請參閱我們的 [ALFA Network 專區](/alfa-network/) 了解用於無線研究的 Wi-Fi 網卡與天線，以及 [Hak5 專區](/hak5/) 了解 WiFi Pineapple 與 USB Rubber Ducky 等滲透測試工具。