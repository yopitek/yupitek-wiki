...
sidebar_position: 15
description: 透過側通道電力分析偵測所有已知惡意 USB 線材的防禦工具 — 包括 O.MG 自家的。同時也是安全充電的資料阻斷器。
tags: [hak5, malicious-cable-detector, defense, usb-security, data-blocker, omg]
keywords: [Malicious Cable Detector, 偵測惡意 USB 線材, 側通道電力分析, O.MG 偵測, USB 資料阻斷器, 200000 次取樣]
authors: yupitek
date: 2026-08-21
...
> **一句話定位**：Malicious Cable Detector 是目前市面上唯一能偵測「所有已知惡意 USB 線材」的防禦型小工具——包括 O.MG Cable 家族——它用每秒 20 萬次的側通道電力分析找出藏線上材裡的植入晶片。同時它也是一顆資料阻斷器，能安全充電。
...
### 步驟 1 — 連線
1. 把可疑 USB 線材插進 **Detector 的**線材端連線埠。
...
| 資料意外穿透 | 焊接跳線設為穿透 | 把跳線改為資料阻斷（見檔案） |
| 線材無法完全插入 | 與笨重聯結器的機械配合問題 | 使用標準外型線材 / 轉接器；用力重新插好 |
...
- [O.MG Cable](/hak5/products/omg-cable/) — 這臺 Detector 就是為了找它而造的植入裝置
- [O.MG UnBlocker](/hak5/products/omg-unblocker/) — *帶有*植入裝置的資料阻斷器（測試你的！）
- [Key Croc](/hak5/products/key-croc/) — 在可疑連線埠上要檢查的鍵盤側錄轉接頭
- [韌體與下載](/hak5/firmware-downloads/) — 檔案與更新資訊
