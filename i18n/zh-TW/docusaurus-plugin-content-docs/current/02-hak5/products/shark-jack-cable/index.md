...
sidebar_position: 7
description: 由 USB-C 供電並附專屬序列主控臺的 Shark Jack — 只要有電就能一直跑的網路偵察，附即時 shell。
tags: [hak5, shark-jack, shark-jack-cable, network-recon, serial-console, usb-c]
keywords: [Shark Jack Cable, USB-C 網路偵察, 序列主控臺, 乙太網路偵察, Kali 實驗室, nmap]
authors: yupitek
date: 2026-08-21
...
額外的收穫是**專屬 USB-C 序列主控臺**。在經典 Shark Jack 上，你要撥到 arming 模式、把線移到筆電、然後 SSH 進入。Cable 版給你一個*透過序列*的即時 shell — 你可以即時觀看掃描進度、與 payload 互動，而且永遠不用把裝置從網路上拿下來。
...
| 電源 | USB-C（只要有供電就能一直跑） |
| 相較經典版新增 | 專屬 USB-C 序列主控臺（即時 shell） |
...
### 步驟 1 — 連線它
1. 把 **USB-C** 端插進電源 / 你的筆電 — 這同時供電並給你序列主控臺。
2. 把 **Ethernet** 端插進目標網路（你的實驗室）。

### 步驟 2 — 選擇模式
- **攻擊模式：** 撥開關；預設 nmap payload 執行並記錄到 `/root/loot/`。
- **Arming 模式：** 撥到另一個位置；透過序列連線，你立刻得到一個 root shell。

### 步驟 3 — 透過序列的即時 shell
不像經典版，你不需要額外的 SSH 跳板就能看到結果。連線 USB-C 序列主控臺並即時互動：
...
| 症狀 | 原因 | 修正 |
|---|---|---|
| 序列主控臺沒顯示 | 序列速度錯誤 / 裝置節點錯誤 | 使用檔案記載的鮑率（115200）與正確的 `/dev/tty*` |
