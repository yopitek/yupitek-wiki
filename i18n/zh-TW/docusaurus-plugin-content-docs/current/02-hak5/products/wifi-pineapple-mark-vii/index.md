...
> **一句話定位**：WiFi Pineapple Mark VII 是一臺「會自己開 Wi-Fi 來釣魚」的無線攻擊平臺 — 它假裝成你信任的無線網路，再用 PineAP 引擎接管受害者的連線。給想學 Wi-Fi 滲透測試、evil twin 攻擊的大學生與 CTF 玩家。
...
> **⚠️ 僅限授權測試。** 只對*你自己的*網路、你自己的裝置，或取得書面許可後執行 Pineapple。在別人的網路上廣播假「免費 Wi-Fi」是違法的（臺灣：刑法第 358–363 條；另見電信法）。
...
| 連線埠 | USB-C（電源 + 乙太網路）、USB 2.0 host |
| 電源 | USB-C 5V 2A（10 W） |
| 指示燈 | 單顆 RGB LED |
| 尺寸 | 107 × 93 × 21 mm |
| OS / UI | 基於 OpenWrt 的韌體，瀏覽器管理介面於連線埠 1471 |
| 官方檔案 | https://docs.hak5.org/wifi-pineapple |
...
| 中間人 | PineAP 從真正的 AP 搶走使用者端；他們的流量經過你 |
| 偵察 | 被動調查附近的 AP 與使用者端（原生 2.4 GHz；搭配 MK7AC 支援 5 GHz） |
...
預期結果 — WiFi Pineapple 儀錶板。**第一件事：** 在 **Settings → Password** 設定你的 admin 密碼。
...
3. 點 **Scan**。幾秒內你會看到附近的 AP 與使用者端填入清單 — Pineapple 在*被動*監聽。
...
| 面板 | 工作 | 典型用途 |
|---|---|---|
| **PineAP** | 引擎：回應 probe request、deauth、冒充 | 開啟「PineAP Daemon」+「Beacon Response」引誘使用者端 |
| **Recon** | 被動 AP/使用者端探索 | 攻擊前調查空域 |
...
3. 從 **Client** 頁面（或一個模組）向目標的使用者端傳送 deauthentication 訊框。
4. 受害者重新連線 — 連到*你的*複製品。開啟 **Client** 面板看他們出現。

> **你可能會問：** *「為什麼受害者會連上假 AP？」* 因為使用者端會持續為它們記得的網路傳送 **probe request**（「Home-5G？」），而 PineAP daemon 會用相符的 beacon 回應每一個 probe。你的複製品看起來一模一樣，所以使用者端選了它。這就是全部的把戲 — 而它之所以有效，是因為 Wi-Fi 使用者端*會廣播自己的歷史*。
...
| 技巧 | 從哪裡開始 |
|---|---|
| 使用者端隔離 / 過濾 | PineAP → Filters：封鎖特定使用者端連到真正的 AP |
...
| 症狀 | 原因 | 修正 |
|---|---|---|
| `PineAP` SSID 看不到 | 還在開機（最多 60 秒） | 等待；檢查 LED。紅燈 = 錯誤 → 透過有線連線檢視日誌 |
| 管理介面載入不了 | 你不在 `172.16.42.0/24` 網路上 | 忘記其他網路；確認你連上了 `PineAP`；重試 `172.16.42.1:1471` |
| 儀錶板沒有網際網路 | 沒有設定 uplink | 把 USB-C 乙太網路接到真實網路；或設定 client 模式 Wi-Fi |
