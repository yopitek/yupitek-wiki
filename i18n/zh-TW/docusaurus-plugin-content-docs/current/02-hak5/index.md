...
> **一句話定位**：Hak5 是全世界最知名的「攻擊型」安全硬體品牌 — 從替別人收發鍵盤敲擊的 USB Rubber Ducky，到假裝成咖啡店 Wi-Fi 的 WiFi Pineapple。本節用大學課程等級的詳細度，帶你一步一步認識每一臺裝置、怎麼設定、怎麼寫 Payload（攻擊載荷）、出了問題怎麼修。
...
> **⚠️ 法律宣告 — 只讀一次。** 本節的所有內容僅限於**授權的安全測試**：你自己的實驗室、你自己的裝置，或你已取得書面授權測試的網路。每個國家都有關於未授權存取的法律（在臺灣，見刑法第 358–363 條與個資法）。未經許可的駭入是犯罪 — 而且用這些工具，也很容易被本頁所介紹的防禦工具偵測到。請在你的沙盒裡玩。
...
1. **Payload 優先於設定** — 你不是「程式化」硬體；你是把一個指令碼檔丟進去。
...
| **WiFi Pineapple Enterprise** | 一臺 1U 機架怪獸，配備 5 組雙頻無線電，適合重度、多目標的空域稽核 | 進階 | [完整指南](/hak5/products/wifi-pineapple-enterprise/) |
...
| **Bash Bunny Mark II** | 多向量 USB 攻擊平臺：鍵盤 + 乙太網路 + 序列埠 + 儲存，一次全上，配備四核大腦 | 中階 | [完整指南](/hak5/products/bash-bunny-mark-ii/) |
...
| **Shark Jack Cable** | 同一臺盒子，改用 USB-C 供電並附序列主控臺 — 只要有電就能一直跑 | 初學者 | [完整指南](/hak5/products/shark-jack-cable/) |
...
| **O.MG Programmer** | 啟用並更新每一臺 O.MG 裝置的通用程式設計器 | 中階 | [完整指南](/hak5/products/omg-programmer/) |
...
3. **架設一個實驗室** — 一臺備用路由器、一臺舊筆電，或一臺你擁有的虛擬機器。[ALFA Network 專區](/alfa-network/)說明如何把 USB Wi-Fi 轉接器搭配 Kali Linux 進入監聽模式，WiFi Pineapple 也很愛這個。
...
> **你可能會問：** *「要學這些，我是不是得全部買下來？」* 不用。這些概念 — HID 注入、rogue AP、網路竊聽 — 都能直接套用到你手上免費的工具：一片兩塊美金的 Arduino 就能打字，`hostapd` 可以偽造 AP，`tcpdump` 可以嗅探。Hak5 硬體只是把它們包裝成足以應付專業任務的可靠方案。從一臺裝置和你的實驗室開始吧。