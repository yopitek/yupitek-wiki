...
> **技術摘要**：以 Rockchip RK3588 / RK3588S 晶片為核心的高效能單板電腦（如 Orange Pi 5 / 5B / 5 Plus、Radxa ROCK 5B）具備強大算力，常被作為邊緣運算無線閘道器與遠程安全感測節點。整合 ALFA 網卡時，**系統镜像文件的核心版本（Vendor BSP vs Armbian Mainline）** 是決定網卡能否「即插即用」的決定性關鍵。

---

## 1. 核心版本差異：Vendor BSP vs Armbian Mainline

許多開發者在 Orange Pi 原廠 Debian 系統上插入 ALFA 網卡後無法辨識，誤以為硬件不相容。實務根因在於作業系統的核心架構差異：
...
flowchart TD
    ...
    subgraph Vendor["原廠 Vendor BSP 系統 (Debian 5.10-rockchip)"]
        ...
        VDrviers["❌ 缺少 mt76 / mt7921u 核心模块<br/>❌ 缺少 /lib/firmware/mediatek 韌體"]
...
1. 下載並刷入適用於 Orange Pi 5 的 **Armbian Linux (Kernel 6.x Mainline)** 镜像文件。
