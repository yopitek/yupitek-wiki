...
在 `wfb-ng` 長距離 FPV 鏈路中，**Realtek RTL8812AU 驅動** 支援將封包長度設為極限、關閉 ACK 握手回應並強行在非標準/特定 5GHz 頻道（如 5.745 GHz，頻道 149）進行無连接單向廣播。ALFA AWUS036ACH 具備獨立功率放大器（PA/LNA）與雙外接天線接口，是目前學術與社群驗證最扎實的長距離 `wfb-ng` 硬件方案。
...
SteamOS 默认採用不可變（Immutable）唯讀文件系統，建置 DKMS 驅動前必須暫時解除唯讀保護：
...
- **A/B 分区覆蓋機制**：SteamOS 每次進行重大系統更新時，會將系統寫入全新 A/B 分区，先前編譯安裝於 `/usr/lib/modules/` 的 DKMS 驅動將會被還原。
